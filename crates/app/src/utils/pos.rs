...
pub fn set_tray_window_pos<R: Runtime>(
  ...
  tracing::info!("Setting tray window position");
  let pos = app.cursor_position()?;
  tracing::info!("Cursor position: {:?}", pos);
  let monitor = window
    .current_monitor()?
  ...
  tracing::info!("Baseline size: {baseline_w_physical}x{baseline_h_physical}");
  let monitor_size = monitor.work_area().size;
  tracing::info!("Monitor size: {:?}", monitor_size);
  // 计算当前窗口显示位置
  ...
  //   tracing::info!("Window position: {x}x{y}");
  ...
  let (x, y) = match (is_left, is_top) {
    ...
    (false, true) => (monitor_size.width - baseline_w_physical, 0u32),
    (false, false) => (
      monitor_size.width - baseline_w_physical,
      monitor_size.height - baseline_h_physical,
    ),
  };
  tracing::info!("Window position: {x}x{y}");
  window.set_position(PhysicalPosition::new(x, y))?;
...
pub fn set_context_window_pos<R: Runtime>(
  ...
  tracing::info!("Setting context window position");
  let pos = app.cursor_position()?;
  tracing::info!("Cursor position: {:?}", pos);
  ...
  tracing::info!("Monitor pos: {:?}", monitor_pos);
