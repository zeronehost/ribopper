  ...
  match store.get("config") {
    Some(config) => {
      let conf: RiboConfig = serde_json::from_value(config).map_err(|e| {
        tracing::error!("commands::config::config_load - parse error: {}", e);
        e
      })?;

      tracing::debug!("commands::config::config_load - returning config");
      Ok(Some(conf))
    }
    None => {
      tracing::debug!("commands::config::config_load - no config found");
      Ok(None)
    }
  }
}
...
pub fn config_save<R: Runtime>(app: AppHandle<R>, config: RiboConfig) -> Result<()> {
  let store = app.store(STORE_FILE).map_err(|e| {
    tracing::error!("commands::config::config_save - failed to open store: {e}");
    e
  })?;
  ...
  tracing::info!("commands::config::config_save - config saved");
...
pub(crate) fn autostart_setting<R: Runtime>(app: &AppHandle<R>, config: &RiboConfig) {
  ...
  match (autostart_manager.is_enabled(), config.get_autostart()) {
    (Ok(false), true) => match autostart_manager.enable() {
      Ok(_) => {
        tracing::info!("commands::config::autostart_setting - autostart enabled");
      }
      Err(e) => {
        tracing::error!(
    ...
    (Ok(true), false) => match autostart_manager.disable() {
      Ok(_) => {
        tracing::info!("commands::config::autostart_setting - autostart disabled");
      }
      Err(e) => {
        tracing::error!(
